...
int findMaxSameAdj(int column, int row, int maxRows, struct Graph* board)
{
	...
	//we want to select a slot to place our chip based on the number of in-a-rows we add to/block.
	//more adjacent same color lines means the slot is more highly contested
	//if we detect 3 in a row of either color adjacent to the slot we're checking, we NEED to place a chip there, because it either secures a victory or prevents an opponent victory
	//we should prefer blue 3 in a rows over red 3 in a rows because we shouldn't block when we can just win.
	//
	int trueMax = 0;
	int c;
	int max = 0;
